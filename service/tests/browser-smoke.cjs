...
(async () => {
    ...
    try {
        ...
        await page.locator('.action-row').first().getByRole('button', {name:'Переместить действие ниже'}).click();
        assert.match(await page.locator('.action-row').first().innerText(), /Поднять груз/);
        await page.locator('.action-row').nth(1).getByRole('button', {name:'Переместить действие выше'}).click();
        assert.match(await page.locator('.action-row').first().innerText(), /Перемещение/);
        assert.equal(await page.locator('.action-row').first().evaluate(el => getComputedStyle(el).borderLeftWidth), '0px');
        assert(await page.locator('.action-row svg use').count() > 0);
        ...
        // Place drone marker so the original drone.png is visible on the map screenshot.
        await page.locator('#mapModeDrone').check();
        const map = page.locator('#mapOverlay');
        const box = await map.boundingBox();
        assert(box, 'map overlay should be visible');
