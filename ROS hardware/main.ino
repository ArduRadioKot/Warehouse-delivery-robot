/****************************************************************************
 * Контроллер базы для робота на ROS2
 * - управление моторами по UART (через команды SET_*)
 * - одометрия по энкодерам
 * - PID-регулятор скоростей колёс
 * - чтение IMU MPU6050 по I2C и вывод показаний в UART
 ***************************************************************************/
 ...
 ***************************************************************************/
bool goingStraight = false;  // Флаг, что робот едет прямо (угловая скорость ≈ 0)
...
// Адрес MPU6050 по I2C (обычно 0x68 при AD0=GND)
#define MPU6050_ADDR             0x68
#define MPU6050_REG_PWR_MGMT_1   0x6B
#define MPU6050_REG_ACCEL_XOUT_H 0x3B
#define MPU6050_REG_GYRO_XOUT_H  0x43

// Глобальные переменные для хранения последних измерений IMU
float imuAx = 0.0f, imuAy = 0.0f, imuAz = 0.0f;   // Ускорения, g
float imuGx = 0.0f, imuGy = 0.0f, imuGz = 0.0f;   // Угловые скорости, рад/с
...
bool initMPU6050() {
  // Выводим датчик из сна
  mpu6050WriteRegister(MPU6050_REG_PWR_MGMT_1, 0x00);
  delay(100);
  // Здесь можно добавить настройку диапазонов и фильтров
  return true;
}

void updateIMU() {
  // Сырые данные акселерометра
  int16_t axRaw = mpu6050Read16(MPU6050_REG_ACCEL_XOUT_H);
  int16_t ayRaw = mpu6050Read16(MPU6050_REG_ACCEL_XOUT_H + 2);
  int16_t azRaw = mpu6050Read16(MPU6050_REG_ACCEL_XOUT_H + 4);

  // Сырые данные гироскопа
  int16_t gxRaw = mpu6050Read16(MPU6050_REG_GYRO_XOUT_H);
  int16_t gyRaw = mpu6050Read16(MPU6050_REG_GYRO_XOUT_H + 2);
  int16_t gzRaw = mpu6050Read16(MPU6050_REG_GYRO_XOUT_H + 4);

  // Преобразуем к физическим единицам
  // По умолчанию MPU6050: аксель ±2g => 16384 LSB/g
  const float accelScale = 1.0f / 16384.0f;
  imuAx = axRaw * accelScale;
  imuAy = ayRaw * accelScale;
  imuAz = azRaw * accelScale;

  // Гироскоп по умолчанию ±250°/s => 131 LSB/(°/s)
  // Переводим в рад/с (для удобства в ROS2)
  const float gyroScaleDeg = 1.0f / 131.0f;
  const float DEG2RAD = PI / 180.0f;
  imuGx = gxRaw * gyroScaleDeg * DEG2RAD;
  imuGy = gyRaw * gyroScaleDeg * DEG2RAD;
  imuGz = gzRaw * gyroScaleDeg * DEG2RAD;
}
...
/****************************************************************************
 * PID-регулятор для колёс с компенсацией прямолинейного движения
 ***************************************************************************/
void computeWheelsPID() {
  ...
  // Ошибки между целевыми и измеренными скоростями
  ...
  if (goingStraight && fabs(targetLeftWheelSpeed) > 1.0 && fabs(targetRightWheelSpeed) > 1.0) {
    ...
    long diff = leftTicks - rightTicks;  // diff > 0 => левое колесо ушло вперёд
    float KencStraight = 3.0f;           // подбирается экспериментально
...
void setRobotVelocity(float linearVelocity, float angularVelocity) {
  // Почти ноль – считаем это остановкой
  ...
  // Обратная кинематика дифференциального привода
...
void processCommand(String command) {
  ...
  if (command.startsWith("SET_PWM")) {
    ...
    float x, y, th;
    // Тут можно реализовать установку позы робота при необходимости
...
void loop() {
  ...
  if (millis() - controlTimer > 50) {   // 20 Гц
    computeSpeed();
    updateOdometry();
    computeWheelsPID();
    updateIMU();
  ...
  if (millis() - printTimer > 200) {    // 5 Гц телеметрия
